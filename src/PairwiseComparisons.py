...
def PairwiseComparisons(arg,mode,id_graphsMono,labelss,keep,TAILLEGRAPHE):
    cv = StratifiedKFold(n_splits=10,shuffle=True,random_state=42)
    
    # Enlever les patterns qui ne sont dans aucun graphe keep
    id_graphs = []
    for i in range(len(id_graphsMono)):
        temp = []
        for j in range(len(id_graphsMono[i])):
            if id_graphsMono[i][j] in keep:
                temp.append(id_graphsMono[i][j])
        if temp!=[]:
            id_graphs.append(temp)
    id_graphsMono = id_graphs
    TAILLEPATTERN = len(id_graphsMono)
    #1) Clustering step
    ...
    for VALUECLUSTER in Range:
        ...
        for score in dicoScores.keys():
            ...
            scoresValues = np.round(scoresValues,10)
        ...
        import scipy
        #create a heatmap of the correlation between the scores
        results = np.zeros((len(dicoScores),len(dicoScores)))
        for i in range(len(dicoScores)):
            for j in range(len(dicoScores)):
                #use rbo to compute the correlation between the rankings
                if METHOD=="RBO":
                    results[i][j]=rbo.RankingSimilarity(dicoRankings[i],dicoRankings[j]).rbo()
                if METHOD=="KT":
                    results[i][j]=scipy.stats.kendalltau(dicoRankings[i],dicoRankings[j])[0]
                #store the correlation in the dataframe
                df.iloc[i][j]=results[i][j]
        NAMEBASE = "../results/"+arg+"/PairwiseComparisons/"
        #if folder does not exist, create it
        if not os.path.exists(NAMEBASE):
            os.makedirs(NAMEBASE)   
        #plot the heatmap
        plt.figure()
        plt.imshow(results, interpolation='nearest',label='Correlation')
        plt.xticks(range(len(dicoScores)),dicoScores.keys(),rotation=90)
        plt.yticks(range(len(dicoScores)),dicoScores.keys())
        plt.colorbar()
        if mode == "c":
            nameSortie = NAMEBASE+"PairwiseComparisons"+str(METHOD)+"Closed"+".pdf"
        if mode == "g":
            nameSortie = NAMEBASE+"PairwiseComparisons"+str(METHOD)+"General"+".pdf"
        if mode == "i":
...
def main(argv):
    ...
    for opt, arg in opts:
        if opt == '-h':
          print ('PairwiseComparisons.py -d <dataset> -m<mode>')
          sys.exit()
        elif opt in ("-d"):
            dataset = arg
        elif opt in ("-m"):
    ...
    PairwiseComparisons(dataset,mode,id_graphs,labels,keep,TAILLEGRAPHE)
